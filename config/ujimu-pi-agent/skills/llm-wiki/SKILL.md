...
## Ujimu project override

When this skill is used by Ujimu's ingestion agent, ingest only Markdown files under `raw/`. Files in `raw/` that do not end in `.md` are original uploads for the conversion pipeline and must not be ingested directly. If a requested source is not a Markdown file under `raw/`, stop and report that conversion to Markdown is required before ingestion.
