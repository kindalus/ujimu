...
describe('Pi chat runtime prompt acceptance', () => {
  ...
  it('uses the specialist root directly and lets Pi answer with optional citations', async () => {
    ...
    expect(prompts[0]).toContain('Follow the llm-wiki Query workflow, including derived-first retrieval.')
    expect(prompts[0]).toContain('Candidate wiki paths from a recent matching consultation, if any:\nwiki/ambito.md')
    expect(prompts[0]).toContain('never override an applicable derived page')
    expect(prompts[0]).toContain('do not read other pages merely to reconfirm it')
