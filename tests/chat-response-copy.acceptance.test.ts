...
describe('assistant response copy acceptance', () => {
  ...
  it('exposes copy controls for user questions and completed assistant responses', async () => {
    ...
    expect(page).toContain("class=\"iconbtn assistant-copy\"")
    expect(page).toContain(":title=\"copiedMessageId === item.message.id ? 'Resposta copiada' : 'Copiar resposta'\"")
    expect(page).toContain(":aria-label=\"copiedMessageId === item.message.id ? 'Resposta copiada' : 'Copiar resposta'\"")
    expect(page).not.toContain("{{ copiedMessageId === item.message.id ? 'Copiado' : 'Copiar resposta' }}")
