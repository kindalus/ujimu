...
describe('admin specialist management UI acceptance', () => {
  it('splits specialist administration into dedicated Nuxt subpages', async () => {
    ...
    expect(detail).toContain('Página pública')
    expect(detail).toContain('Título SEO')
    expect(detail).toContain('Meta-descrição')
    expect(detail).toContain('Introdução pública')
    expect(detail).toContain('Temas abrangidos')
    expect(detail).toContain('Limites da especialidade')
    expect(detail).toContain('Chamada para consulta')
