...
| 07 | [`07-conversation-history-editing.html`](./07-conversation-history-editing.html) | `idea-refined` | — | Direction refined; ready for grill-me implementation decisions. |
...
Key assumptions to validate during grilling:

- Whether AI title generation should be synchronous, backgrounded, or use a temporary placeholder.
- Whether a resumed conversation can continue only under its original specialist, with no specialist switching.
- Whether message editing is limited to authenticated user-authored messages.
- How to preserve citation traceability when persisted assistant messages are restored.
